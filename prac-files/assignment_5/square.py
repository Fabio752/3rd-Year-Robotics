...
class robot:
    ...
    def rotate_top(self):
        rad_amount = 4.15
        rad_interval = rad_amount / 36
        speed_dps = 45
        BP.offset_motor_encoder(BP.PORT_A, BP.get_motor_encoder(BP.PORT_A))
        print("encoder value: ", BP.get_motor_encoder(BP.PORT_A))
        # Rotate robot left by 90 degrees.
        while rad_amount > 0:
            actual_degree = 0
            ...
            while actual_degree < target_rotation:
                # Actual degree negate
                actual_degree = -BP.get_motor_encoder(BP.PORT_A)
                # print_robot_stats(BP.PORT_A)
                # print_robot_stats(BP.PORT_B)
                time.sleep(0.02)
                #print("Rotation complete")
            #Measurement and calculations should be sone here
            self.stop_robot()
            time.sleep(1)
