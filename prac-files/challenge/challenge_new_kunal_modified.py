...
class robot:
    ...
    def set_estimate_x_by_hand(self, x):
        ...
        self.estimate_location = [x, self.estimate_location[1], self.estimate_location[2]]
    
    def set_estimate_y_by_hand(self, y):
        print("setting manually y to: ", y)
        self.estimate_location[1] = y
        ...
        while actual_degree_rotation < target_degree_rotation:
            ...
            while True:
                try:
                    ...
                    sonar_value = self.measure_sonar(4)
    ...
    def go_to_bottle_2(self):
        ...
        if forward_val < threshold:
            ...
            self.set_estimate_x_by_hand(180 - initial_distance)
            if initial_distance > threshold:
                ...
                if -8 < difference and difference < 0:
                    self.rotate(-0.02 * difference, 180)
                elif 0 < difference and difference < 8:
                    self.rotate(0.02 * difference , 180)
   
                seen_object = self.forward_till_hit_w_sonar(190 - self.get_estimate_location () [1], 300, 40)
                self.rotate_sonar(math.pi/2, -270) 
            else :
                seen_object = True
                self.rotate_sonar(math.pi/2, -270)
    ...
    def go_to_bottle_3(self):
        ...
        print("attempt correction, sonar value = ", sensor_value)
        self.rotate(0.05, 180)
        new_sensor_value = self.measure_sonar(2)
        if (new_sensor_value - sensor_value) > 1 :
            print("fix wrong try, new sensor value = ", new_sensor_value)
            self.rotate(-0.05, 180)
        ...
        if sensor_value < 85:
            ...
            self.rotate_sonar(-math.pi/2, -270)
            seen_object = r.forward_till_hit_w_sonar(70,  300, 75)

        if seen_object:
            time.sleep(1) #Going a bit forward DO NOT REMOVE
            self.rotate(-math.pi/2, 90)
            ...
            self.go_forward(25, -350)
            final_value = self.measure_sonar(4)
            print("final_value", final_value)
            difference = initial_value - final_value
            if(0 < difference) and (difference < 8):
                self.rotate(0.02 * difference, 180)
            elif (-8 < difference) and (difference < 0):
                self.rotate(-0.02 * difference, 180)
            ...
            self.go_forward(y_coord - 30, -300)
            #Read x coordinate
...
try:
    ...
    r.go_forward(94 - r.estimate_location[1], 350)
